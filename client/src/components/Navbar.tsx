  ...
  return (
    <>
      <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800">
        <Link to='/'>
          <img src={assets.logo} alt="logo" className='h-8 sm:h-11' />
        </Link >
        ...
        <div className="flex items-center gap-3">
          ...
          {!session?.user ? (
            ...
            <>
              ...
              <UserButton size='icon' className='w-9 h-9' />
            </>
          )
          }
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="fixed inset-0 z-[100px] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
          <Link to='/' onClick={() => setMenuOpen(false)}>Home</Link>
          <Link to='/projects' onClick={() => setMenuOpen(false)}>My Products</Link>
