...
const Sidebar = ({isMenuOpen, project, setProject, isGenerating, setIsGenerating} : SidebarProps) => {
    ...
    const fetchProject = async () => {
        try {
            ...
            toast.error(error?.respose?.data?.message || error.messsage);
            console.log(error);
        }
    }
    ...
    const handleRevisions = async (e: React.SyntheticEvent<HTMLFormElement>)=> {
        e.preventDefault()
  ...
  return (
    <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
      <div className='flex flex-col h-full'>
        {/* Message container */}
        <div className='flex-1 overflow-y-auto px-3 flex flex-col gap-4 no-scrollbar'>
                ...
                if(isMessage){
                    ...
                    return(
                        <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                            ...
                            <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : "rounded-tl-none bg-gray-800 text-gray-100"}`}>
                                {msg.content}
                            </div>
                            {isUser && (
