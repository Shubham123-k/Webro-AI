...
const Projects = () => {
  ...
  const toggelPublish = async () => {
    try {
      const { data } = await api.get(`/api/project/publish-toggle/${projectId}`);
      toast.success(data.message)
  ...
  return project ? (
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      {/* builder navbar */}
      <div className='flex max-sm:flex-col sm:itmes-center gap-4 px-4 py-2 no-scrollbar'>
        {/* left */}
        ...
        <div className='hidden sm:flex gap-2 bg-gray-950 p-1.5 rounded-md'>
          <SmartphoneIcon onClick={() => setDevice('phone')} className={`size-6 rounded cursor-pointer ${device == 'phone' ? "bg-gray-700" : ""}`} />

          <TabletIcon onClick={() => setDevice('tablet')} className={`size-6 rounded cursor-pointer ${device == 'tablet' ? "bg-gray-700" : ""}`} />

          <LaptopIcon onClick={() => setDevice('desktop')} className={`size-6 rounded cursor-pointer ${device == 'desktop' ? "bg-gray-700" : ""}`} />
        </div>
        {/* right */}
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          ...
          <button onClick={toggelPublish} className='bg-linear-to-br from-indigo-700 to-indigo-600 hover:from-indigo-600 hover:to-indigo-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors'>
            {project.isPublished ?
