...
// Controller for the new project
...
// Controllers for to get all the users project
...
export const togglelPublish = async (req: Request, res: Response) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ message: 'Unaithorized' });
...
// Controller Fuction to purchase credits
export const purchaseCredits = async (req: Request, res: Response) => {
    try {
        ...
        const plans = {
            ...
            entrprises: { credits: 1000, amount: 49 },
        }
            ...
            line_items: [
                {
                    price_data: {
                        currency: 'usd',
                        product_data: {
                            name: `Webro AI - ${plan.credits} credits`
                        },
